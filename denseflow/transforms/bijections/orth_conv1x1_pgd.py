...
class OrthogonalConv1x1Pgd(Bijection):
    """
    ...
    def __init__(self, num_channels, orthogonal_init=True, slogdet_cpu=True, trainable=True):
        super(OrthogonalConv1x1Pgd, self).__init__()
    ...
    def orthogonalize_kernel(self):
        if not self.trainable:
            return
        with torch.no_grad():
            w = self.weight.data.squeeze().clone()
            weight = self.weight.data.squeeze()
            weight = orthogonalize_tensor(weight)
            # weight = torch.qr(weight)[0]
            # print((w - weight).abs().max())
